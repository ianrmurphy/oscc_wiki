...
The brake firmware is responsible for reading values from the brake pedal sensor, sending reports on its state, and receiving brake commands and fault reports from the control CAN. When the brake firmware receives a brake command message, it will then output the commanded high and low spoof signals onto its connection to the ECU, sending it brake requests. Receiving a fault report will cause the brake module to disable.
